...
class RenderThread(threading.Thread):
    ...
    def run(self):
        ...
        for idx in self.idxs:
            ...
            frame = Frame(self.width, self.height, self.xrange, self.yrange, BLACK)
            for i in range(trail.shape[1]):
                for p in range(trail.shape[0]):
                    x, y = trail[p][i]
                    o = int((i/trail.shape[1])*255)
                    # print("Opacity:", o, "i:", i, "trail length:", trail.shape[1])
                    frame.addPoint(x, y, color=self.color, opacity=o, radius=0.6)
            frame.renderPoints()
